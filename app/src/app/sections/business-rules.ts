...
@Component({
  ...
  <div class="page">
    <div class="summary">
      ...
      <div class="sum info"><span class="n">{{ summary.totalChecks }}</span><span>Reglas</span></div>
    </div>
    ...
    <div class="card group" *ngFor="let g of groups">
      ...
      <table class="tbl" *ngIf="g.items.length; else vacio">
        <thead><tr><th>Regla</th><th>Detalle</th><th>Impacto</th><th>Afectados</th></tr></thead>
        <tbody>
...
export class BusinessRules {
  private readonly checks = BUSINESS_RULES.map(rule => {
    const affected = CONTRATOS.filter(rule.evaluate).length;
    const resultado = affected === 0 ? 'ok' : rule.severity;
    const detalle = affected === 0 ? 'Sin problemas detectados en esta regla.' : rule.detail(affected);
    return {
  ...
  readonly summary = {
    okCount: this.checks.filter(c => c.resultado === 'ok').length,
    warningCount: this.checks.filter(c => c.resultado === 'warning').length,
    errorCount: this.checks.filter(c => c.resultado === 'error').length,
    totalChecks: this.checks.length,
  };

  readonly groups = [
    { nivel: 'error', titulo: 'Errores', items: this.checks.filter(c => c.resultado === 'error') },
    { nivel: 'warning', titulo: 'Warnings', items: this.checks.filter(c => c.resultado === 'warning') },
    { nivel: 'ok', titulo: 'OK', items: this.checks.filter(c => c.resultado === 'ok') },
  ];
}
